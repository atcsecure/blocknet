...
struct vote_instance_t {
    ...
    IMPLEMENT_SERIALIZE(
        ...
        if(ser_action.ForRead()) {
            const_cast<vote_instance_t *>(this)->eOutcome = vote_outcome_enum_t(nOutcome);
        }
    )
...
public:
    ...
    json_spirit::Object GetJSONObject();
    ...
    IMPLEMENT_SERIALIZE(
        ...
        if(nType & SER_DISK) {
            ...
            printf("CGovernanceObject::SerializationOp Reading/writing votes from/to disk\n");
            READWRITE(nDeletionTime);
            READWRITE(fExpired);
            READWRITE(mapCurrentMNVotes);
            READWRITE(fileVotes);
            printf("CGovernanceObject::SerializationOp hash = %s, vote count = %d\n", GetHash().ToString(), fileVotes.GetVoteCount());
        }
...
private:
    // FUNCTIONS FOR DEALING WITH DATA STRING
    void LoadData();
    void GetData(json_spirit::Object & objResult);
