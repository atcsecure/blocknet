...
bool CMasternodeSync::IsBlockchainSynced(bool fBlockAccepted)
{
    ...
    if(fBlockAccepted)
    {
        ...
        if(!IsSynced())
        {
            ...
            if(fDebug)
            {
                printf("CMasternodeSync::IsBlockchainSynced -- reset\n");
            }
    ...
    if(fDebug)
    {
        printf("CMasternodeSync::IsBlockchainSynced -- state before check: %ssynced, skipped %d times\n",
...
void CMasternodeSync::ProcessTick()
{
    ...
    if(fDebug)
    {
        printf("CMasternodeSync::ProcessTick -- nTick %d nMnCount %d\n", nTick, nMnCount);
    }
    ...
    // if (!IsBlockchainSynced() && nRequestedMasternodeAssets > MASTERNODE_SYNC_SPORKS)
    if (!IsBlockchainSynced() && nRequestedMasternodeAssets > MASTERNODE_SYNC_LIST)
    {
...
//        }
        ...
        // NORMAL NETWORK MODE - TESTNET/MAINNET
        {
            if(netfulfilledman.HasFulfilledRequest(pnode->addr, "full-sync"))
            {
                ...
                printf("CMasternodeSync::ProcessTick -- disconnecting from recently synced peer %s\n", pnode->addr.ToString().c_str());
                continue;
            }
...
//                printf("CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- requesting sporks from peer %s\n",
//                       nTick, nRequestedMasternodeAssets, pnode->addr.ToString().c_str());
