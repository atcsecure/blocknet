...
bool CMasternode::UpdateFromNewBroadcast(CMasternodeBroadcast& mnb)
{
    ...
    if(fMasterNode && pubKeyMasternode == activeMasternode.pubKeyMasternode)
    {
        ...
        else
        {
            ...
            printf("CMasternode::UpdateFromNewBroadcast -- wrong PROTOCOL_VERSION, re-activate your MN: message nProtocolVersion=%d  PROTOCOL_VERSION=%d\n", nProtocolVersion, PROTOCOL_VERSION);
            return false;
        }
    }
    return true;
...
void CMasternode::Check(bool fForce)
{
    ...
    printf("CMasternode::Check -- Masternode %s is in %s state\n", vin.prevout.ToString().c_str(), GetStateString().c_str());
...
//        nHeight = chainActive.Height();
    ...
    if(IsPoSeBanned())
    {
        ...
        printf("CMasternode::Check -- Masternode %s is unbanned and back in list now\n", vin.prevout.ToString().c_str());
    ...
    else if(nPoSeBanScore >= MASTERNODE_POSE_BAN_MAX_SCORE)
    {
        ...
        printf("CMasternode::Check -- Masternode %s is banned till block %d now\n", vin.prevout.ToString().c_str(), nPoSeBanHeight);
        return;
    }
    ...
    if(fRequireUpdate)
    {
        ...
        {
            printf("CMasternode::Check -- Masternode %s is in %s state now\n", vin.prevout.ToString().c_str(), GetStateString().c_str());
        }
        return;
    }
    ...
    if(fWaitForPing && !fOurMasternode)
    {
        ...
        {
            printf("CMasternode::Check -- Masternode %s is in %s state, waiting for ping\n", vin.prevout.ToString().c_str(), GetStateString().c_str());
            return;
        }
    }
    ...
    if(!fWaitForPing || fOurMasternode)
    {
        ...
        printf("CMasternode::Check -- outpoint=%s, nTimeLastWatchdogVote=%d, GetTime()=%d, fWatchdogExpired=%d\n",
...
//                    printf("CMasternode::UpdateLastPaidBlock -- searching for block with payment to %s -- found new %d\n", vin.prevout.ToString().c_str(), nBlockLastPaid);
//                    return;
//                }
//            }
//        }
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast &mnbRet, bool fOffline)
{
    ...
    if(!DataSigner::GetKeysFromSecret(strKeyMasternode, keyMasternodeNew, pubKeyMasternodeNew))
    {
...
//    }
    ...
    printf("CMasternodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyMasternodeNew.GetID() = %s\n",
             CBitcoinAddress(pubKeyCollateralAddressNew.GetID()).ToString().c_str(),
             pubKeyMasternodeNew.GetID().ToString().c_str());

    CMasternodePing mnp(txin);
    if(!mnp.Sign(keyMasternodeNew, pubKeyMasternodeNew))
    {
        strErrorRet = strprintf("Failed to sign ping, masternode=%s", txin.prevout.ToString());
        printf("CMasternodeBroadcast::Create -- %s\n", strErrorRet.c_str());
    ...
    {
        strErrorRet = strprintf("Invalid IP address, masternode=%s", txin.prevout.ToString());
        printf("CMasternodeBroadcast::Create -- %s\n", strErrorRet.c_str());
        mnbRet = CMasternodeBroadcast();
        return false;
    }
    ...
    {
        strErrorRet = strprintf("Failed to sign broadcast, masternode=%s", txin.prevout.ToString());
        printf("CMasternodeBroadcast::Create -- %s\n", strErrorRet.c_str());
        mnbRet = CMasternodeBroadcast();
        return false;
    }
...
bool CMasternodeBroadcast::SimpleCheck(int& nDos)
{
    ...
    if(!IsValidNetAddr())
    {
        printf("CMasternodeBroadcast::SimpleCheck -- Invalid addr, rejected: masternode=%s  addr=%s\n",
    ...
    {
        printf("CMasternodeBroadcast::SimpleCheck -- Signature rejected, too far into the future: masternode=%s\n",
               vin.prevout.ToString().c_str());
        nDos = 1;
        return false;
    }
    ...
    {
        printf("CMasternodeBroadcast::SimpleCheck -- ignoring outdated Masternode: masternode=%s  nProtocolVersion=%d\n", vin.prevout.ToString().c_str(), nProtocolVersion);
        return false;
    }
    ...
    {
        printf("CMasternodeBroadcast::SimpleCheck -- pubKeyCollateralAddress has the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    {
        printf("CMasternodeBroadcast::SimpleCheck -- pubKeyMasternode has the wrong size\n");
        nDos = 100;
        return false;
    }
    ...
    {
        printf("CMasternodeBroadcast::SimpleCheck -- Ignore Not Empty ScriptSig %s\n", vin.ToString().c_str());
        nDos = 100;
        return false;
    }
...
bool CMasternodeBroadcast::Update(CMasternode* pmn, int& nDos)
{
    ...
    {
        printf("CMasternodeBroadcast::Update -- Bad sigTime %d (existing broadcast is at %d) for Masternode %s %s\n",
    ...
    if(pmn->IsPoSeBanned())
    {
        printf("CMasternodeBroadcast::Update -- Banned by PoSe, masternode=%s\n", vin.prevout.ToString().c_str());
        return false;
    }
    ...
    {
        printf("CMasternodeBroadcast::Update -- Got mismatched pubKeyCollateralAddress and vin\n");
        nDos = 33;
        return false;
    }
    ...
    {
        printf("CMasternodeBroadcast::Update -- CheckSignature() failed, masternode=%s\n", vin.prevout.ToString().c_str());
        return false;
    }
    ...
    if(!pmn->IsBroadcastedWithin(MASTERNODE_MIN_MNB_SECONDS) || (fMasterNode && pubKeyMasternode == activeMasternode.pubKeyMasternode))
    {
        ...
        printf("CMasternodeBroadcast::Update -- Got UPDATED Masternode entry: addr=%s\n", addr.ToString().c_str());
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (!CheckSignature(nDos))
    {
        printf("CMasternodeBroadcast::CheckOutpoint -- CheckSignature() failed, masternode=%s\n", vin.prevout.ToString().c_str());
        return false;
    }

    {
        ...
        if(!lockMain)
        {
            ...
            printf("CMasternodeBroadcast::CheckOutpoint -- Failed to aquire lock, addr=%s", addr.ToString().c_str());
...
//            return false;
//        }
//        if(coins.vout[vin.prevout.n].nValue != MASTERNODE_AMOUNT * COIN) {
//            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %d BLOCK, masternode=%s\n", MASTERNODE_AMOUNT, vin.prevout.ToString());
//            return false;
//        }
...
//        }
    ...
    printf("CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO verified\n");
    ...
    {
        printf("CMasternodeMan::CheckOutpoint -- Got mismatched pubKeyCollateralAddress and vin\n");
        nDos = 33;
        return false;
    }

    // verify that sig time is legit in past
    // should be at least not earlier than block when 1000 BLOCKS tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, hashBlock);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second)
        {
            ...
            CBlockIndex * pConfIndex = FindBlockByHeight(pMNIndex->nHeight + nMasternodeMinimumConfirmations - 1);
            if(!pConfIndex || pConfIndex->GetBlockTime() > sigTime)
            {
                printf("CMasternodeBroadcast::CheckOutpoint -- Bad sigTime %d (%d conf block is at %d) for Masternode %s %s\n",
                       sigTime, nMasternodeMinimumConfirmations,
...
bool CMasternodeBroadcast::Sign(CKey& keyCollateralAddress)
{
    ...
    {
        printf("CMasternodeBroadcast::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!DataSigner::VerifyMessage(pubKeyCollateralAddress, vchSig, strMessage, strError))
    {
        printf("CMasternodeBroadcast::Sign -- VerifyMessage() failed, error: %s\n", strError.c_str());
        return false;
    }
...
bool CMasternodeBroadcast::CheckSignature(int& nDos)
{
    ...
    printf("CMasternodeBroadcast::CheckSignature -- strMessage: %s  pubKeyCollateralAddress address: %s  sig: %s\n",
           strMessage.c_str(), CBitcoinAddress(pubKeyCollateralAddress.GetID()).ToString().c_str(),
           EncodeBase64(&vchSig[0], vchSig.size()).c_str());

    if(!DataSigner::VerifyMessage(pubKeyCollateralAddress, vchSig, strMessage, strError))
    {
        printf("CMasternodeBroadcast::CheckSignature -- Got bad Masternode announce signature, error: %s\n", strError.c_str());
        nDos = 100;
        return false;
    }
...
bool CMasternodePing::Sign(CKey& keyMasternode, CPubKey& pubKeyMasternode)
{
    ...
    if (!DataSigner::SignMessage(strMessage, vchSig, keyMasternode))
    {
        printf("CMasternodePing::Sign -- SignMessage() failed\n");
        return false;
    }

    if (!DataSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError))
    {
        printf("CMasternodePing::Sign -- VerifyMessage() failed, error: %s\n", strError.c_str());
        return false;
    }
...
bool CMasternodePing::CheckSignature(CPubKey& pubKeyMasternode, int &nDos)
{
    ...
    if (!DataSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError))
    {
        printf("CMasternodePing::CheckSignature -- Got bad Masternode ping signature, masternode=%s, error: %s\n",
...
bool CMasternodePing::SimpleCheck(int& nDos)
{
    ...
    if (sigTime > GetAdjustedTime() + 60 * 60)
    {
        printf("CMasternodePing::SimpleCheck -- Signature rejected, too far into the future, masternode=%s\n", vin.prevout.ToString().c_str());
        nDos = 1;
        return false;
    }

    {
        ...
        {
            printf("CMasternodePing::SimpleCheck -- Masternode ping is invalid, unknown block hash: masternode=%s blockHash=%s\n",
    ...
    printf("CMasternodePing::SimpleCheck -- Masternode ping verified: masternode=%s  blockHash=%s  sigTime=%d\n",
...
bool CMasternodePing::CheckAndUpdate(CMasternode* pmn, bool fFromNewBroadcast, int& nDos)
{
    ...
    if (pmn == NULL)
    {
        printf("CMasternodePing::CheckAndUpdate -- Couldn't find Masternode entry, masternode=%s\n", vin.prevout.ToString().c_str());
        return false;
    }
    ...
    if(!fFromNewBroadcast)
    {
        ...
        {
            printf("CMasternodePing::CheckAndUpdate -- masternode protocol is outdated, masternode=%s\n", vin.prevout.ToString().c_str());
            return false;
        }

        if (pmn->IsNewStartRequired())
        {
            printf("CMasternodePing::CheckAndUpdate -- masternode is completely expired, new start is required, masternode=%s\n", vin.prevout.ToString().c_str());
            return false;
        }
    }

    {
        ...
        {
            printf("CMasternodePing::CheckAndUpdate -- Masternode ping is invalid, block hash is too old: masternode=%s  blockHash=%s\n",
                   vin.prevout.ToString().c_str(), blockHash.ToString().c_str());
            // nDos = 1;
            return false;
        }
    ...
    printf("CMasternodePing::CheckAndUpdate -- New ping: masternode=%s  blockHash=%s  sigTime=%d\n",
           vin.prevout.ToString().c_str(), blockHash.ToString().c_str(), sigTime);
    ...
    {
        printf("CMasternodePing::CheckAndUpdate -- Masternode ping arrived too early, masternode=%s\n",
               vin.prevout.ToString().c_str());
    ...
    if(!masternodeSync.IsMasternodeListSynced() && !pmn->IsPingedWithin(MASTERNODE_EXPIRATION_SECONDS/2))
    {
        ...
        printf("CMasternodePing::CheckAndUpdate -- bumping sync timeout, masternode=%s\n", vin.prevout.ToString().c_str());
        masternodeSync.AddedMasternodeList();
    }

    // let's store this ping as the last one
    printf("CMasternodePing::CheckAndUpdate -- Masternode ping accepted, masternode=%s\n", vin.prevout.ToString().c_str());
    pmn->lastPing = *this;
    ...
    printf("CMasternodePing::CheckAndUpdate -- Masternode ping acceepted and relayed, masternode=%s\n",
           vin.prevout.ToString().c_str());
    Relay();
