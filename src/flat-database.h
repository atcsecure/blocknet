...
private:
    ...
    ReadResult Read(T& objToLoad, bool fDryRun = false)
    {
        ...
        printf("Loaded info from %s  %dms\n", strFilename.c_str(), GetTimeMillis() - nStart);
        printf("     %s\n", objToLoad.ToString().c_str());
        if(!fDryRun) {
            printf("%s: Cleaning....\n", __func__);
...
public:
    ...
    bool Load(T& objToLoad)
    {
        ...
        if (readResult == FileError)
            printf("Missing file %s, will try to recreate\n", strFilename.c_str());
        else if (readResult != Ok)
        {
            printf("Error reading %s: ", strFilename.c_str());
            if(readResult == IncorrectFormat)
            {
                printf("%s: Magic is ok but data has invalid format, will try to recreate\n", __func__);
            }
            else {
                printf("%s: File format is unknown or invalid, please fix it manually\n", __func__);
    ...
    bool Dump(T& objToSave)
    {
        ...
        printf("Writting info to %s...\n", strFilename.c_str());
        Write(objToSave);
        printf("%s dump finished  %dms\n", strFilename.c_str(), GetTimeMillis() - nStart);
