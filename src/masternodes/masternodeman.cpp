...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    for (CNode * pnode : vNodes) {
        if(pnode->fMasternode) {
            ...
            printf("Closing Masternode connection: peer=%d, addr=%s\n", pnode->id, pnode->addr.ToString());
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::MNANNOUNCE) { //Masternode Broadcast
            ...
            if(!isLocal && Params().NetworkIDString() == CBaseChainParams::MAIN) {
                ...
                if (i != mAskedUsForMasternodeList.end()){
                    ...
                    if (GetTime() < t) {
                        ...
                        printf("DSEG -- peer already asked me for the list, peer=%d\n", pfrom->id);
                        return;
                    }
                }
                int64_t askAgain = GetTime() + DSEG_UPDATE_SECONDS;
        ...
        for (CMasternode & mn : vMasternodes) {
            ...
            if (vin == mn.vin) {
                printf("DSEG -- Sent 1 Masternode inv to peer %d\n", pfrom->id);
                return;
            }
        }
        ...
        if(vin == CTxIn()) {
            ...
            printf("DSEG -- Sent %d Masternode invs to peer %d\n", nInvCount, pfrom->id);
            return;
        }
        // smth weird happen - someone asked us for vin we have no idea about?
        printf("DSEG -- No invs sent to peer %d\n", pfrom->id);
...
void CMasternodeMan::DoFullVerificationStep()
{
    ...
    while(it != vecMasternodeRanks.end()) {
        ...
        if(it->second.vin == activeMasternode.vin) {
            nMyRank = it->first;
            printf("CMasternodeMan::DoFullVerificationStep -- Found self at rank %d/%d, verifying up to %d masternodes\n",
...
void CMasternodeMan::SendVerifyReply(CNode* pnode, CMasternodeVerification& mnv)
{
    ...
    if(netfulfilledman.HasFulfilledRequest(pnode->addr, strprintf("%s", NetMsgType::MNVERIFY)+"-reply")) {
        ...
        printf("MasternodeMan::SendVerifyReply -- ERROR: peer already asked me recently, peer=%d\n", pnode->id);
    ...
    if(!GetBlockHash(blockHash, mnv.nBlockHeight)) {
        printf("MasternodeMan::SendVerifyReply -- can't get block hash for unknown block height %d, peer=%d\n", mnv.nBlockHeight, pnode->id);
        return;
    }
...
void CMasternodeMan::ProcessVerifyReply(CNode* pnode, CMasternodeVerification& mnv)
{
    ...
    if(!netfulfilledman.HasFulfilledRequest(pnode->addr, strprintf("%s", NetMsgType::MNVERIFY)+"-request")) {
        printf("CMasternodeMan::ProcessVerifyReply -- ERROR: we didn't ask for verification of %s, peer=%d\n", pnode->addr.ToString(), pnode->id);
        pnode->Misbehaving(20);
        return;
    }
    ...
    if(mWeAskedForVerification[pnode->addr].nonce != mnv.nonce) {
        printf("CMasternodeMan::ProcessVerifyReply -- ERROR: wrong nounce: requested=%d, received=%d, peer=%d\n",
    ...
    if(mWeAskedForVerification[pnode->addr].nBlockHeight != mnv.nBlockHeight) {
        printf("CMasternodeMan::ProcessVerifyReply -- ERROR: wrong nBlockHeight: requested=%d, received=%d, peer=%d\n",
    ...
    if(!GetBlockHash(blockHash, mnv.nBlockHeight)) {
        ...
        printf("MasternodeMan::ProcessVerifyReply -- can't get block hash for unknown block height %d, peer=%d\n", mnv.nBlockHeight, pnode->id);
        return;
    }
...
void CMasternodeMan::ProcessVerifyBroadcast(CNode* pnode, const CMasternodeVerification& mnv)
{
    ...
    if(mnv.nBlockHeight < pCurrentBlockIndex->nHeight - MAX_POSE_BLOCKS) {
        printf("MasternodeMan::ProcessVerifyBroadcast -- Outdated: current block %d, verification block %d, peer=%d\n",
                    pCurrentBlockIndex->nHeight, mnv.nBlockHeight, pnode->id);
        return;
    }

    if(mnv.vin1.prevout == mnv.vin2.prevout) {
        printf("MasternodeMan::ProcessVerifyBroadcast -- ERROR: same vins %s, peer=%d\n",
    ...
    if(!GetBlockHash(blockHash, mnv.nBlockHeight)) {
        // this shouldn't happen...
        printf("MasternodeMan::ProcessVerifyBroadcast -- Can't get block hash for unknown block height %d, peer=%d\n", mnv.nBlockHeight, pnode->id);
        return;
    }
    ...
    if(nRank > MAX_POSE_RANK) {
        printf("CMasternodeMan::ProcessVerifyBroadcast -- Mastrernode %s is not in top %d, current rank %d, peer=%d\n",
