...
CBigNum bnProofOfStakeLimit(~uint256() >> 20);
CBigNum bnProofOfWorkLimitTestNet(~uint256() >> 20);

unsigned int nTargetSpacing = 1 * 90; // 90 seconds
unsigned int nStakeMinAge = 60 * 60 * 4; // 4 hours
unsigned int nStakeMaxAge = -60 * 60 * 24 * 30 ; // 20 days
...
const string strMessageMagic = "blocknet Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "blocknet", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x3b7745f385ba47d9b1d3ec7ae31d6a7665e21803081e06e5d844a8a8503d61c6"));
        block.print();
...
std::vector<std::string> getLocalBitcoinAddresses()
{
...

    {
        ...
        for (const PAIRTYPE(CTxDestination, std::string) & item : pwalletMain->mapAddressBook)
        {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::NETVERSION)
    {
...
//                {
//                    printf("ProcessGetData -- MSG_GOVERNANCE_OBJECT: inv = %s\n", inv.ToString().c_str());
//                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
//                    bool topush = false;
//                    {
...
//                        }
//                    }
//                    printf("ProcessGetData -- MSG_GOVERNANCE_OBJECT: topush = %d, inv = %s\n", topush, inv.ToString().c_str());
//                    if(topush)
//                    {
...
//                    if(topush)
//                    {
//                        printf("ProcessGetData -- pushing: inv = %s\n", inv.ToString().c_str());
...
//    }
    ...
    else
    {
        ...
        else
        {
            ...
            // printf("Unknown command \"%s\" from peer=%s\n", SanitizeString(strCommand).c_str(), pfrom->addr.ToString().c_str());
        }
    }
